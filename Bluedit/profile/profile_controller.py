...
def image_change():
    if request.method == "POST":
        ...
        if request.files["image"].filename != '':
            ...
            folder_identity = hashlib.sha1(str(current_user.id).encode()).hexdigest()
            ...
            user_logger.info("User " + current_user.id + "updated his/her profile image. File is stored at - " + store_path)
...
def manage_profile_update_name():
    if request.method == "POST":
        ...
        user_logger.info("User " + current_user.id + "updated his/her username from " + current_user.name + " to " + uname)
