...
@auth_bp.route('/auth/management/delete_account', methods=["POST"])
@login_required
def manage_profile_delete_account():
    if request.method == "POST":
        # 1) Declaring variables
        challenge_text = "I wish to delete my account now"
        input_text = request.form["delete-txt"]
        err_msg = "Input does not match the challenge text"
        success_msg = "Delete success. Your account have been deleted from our system"

        # 2) Validating text input
        if challenge_text != input_text:
            session["profile_manage_auth_status"] = ["null", err_msg, "alert-danger"]
            return redirect(url_for('profile_bp.manage_profile'))

        # 3) Delete user profile image folder
        folder = Config.IMAGE_UPLOADS + "profile_image/" + current_user.id
        shutil.rmtree(folder)

        # 4) Delete user account
        delete_account = auth_manager.delete_account(current_user.id)
        if delete_account == -1: abort(500)
...
# -----------------------------------------------------------------------
# User callback for login manager
# -----------------------------------------------------------------------
